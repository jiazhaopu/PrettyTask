...
public class ZookeeperClient implements AutoCloseable, DisposableBean {
  ...
  public ZookeeperClient(Properties props) {
    String zookeeperConnect = props.getProperty("zookeeper.connect");
    this.zookeeperRootPath = props.getProperty("zookeeper.root_path", ROOT_PATH_DEFAULT);
    int zookeeperSessionTimeout =
      Integer.parseInt(props.getProperty("zookeeper.session.timeout", String.valueOf(ZOOKEEPER_SESSION_TIMEOUT)));
    int zookeeperConnectionTimeout =
      Integer.parseInt(props.getProperty("zookeeper.connection.timeout", String.valueOf(ZOOKEEPER_CONNECT_TIMEOUT)));
    int zookeeperBaseRetryInterval =
      Integer.parseInt(props.getProperty("zookeeper.retry.interval", String.valueOf(CURATOR_BASE_RETRY_INTERVAL)));
    int zookeeperRetryTimes =
      Integer.parseInt(props.getProperty("zookeeper.retry.times", String.valueOf(CURATOR_RETRY_TIMES)));
    this.lockMap = new HashMap<>();
    try {
//      KerberosUtils.autoLogin();
      log.info("zookeeper login kerberos successful");
    } catch (Exception e) {
      log.warn("failed to autoLogin in zookeeper client", e);
    }
